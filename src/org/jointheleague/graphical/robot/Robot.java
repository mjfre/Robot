 ...
 * </p>
 * <p>
 * A Robot is controlled by calling its {@link #move(int)},
 * {@link #turn(double)}, {@link #microMove(int)}, and {@link #microTurn(int)}
 * methods. These methods should be called from the same thread, which is
 * typically the main thread, but different Robots may be controlled from
 * different threads, thereby allowing the Robots to move simultaneously.
 * </p>
 * <p>
...
public class Robot implements RobotInterface {
	 ...
	 *
	 * @param fileName
	 *                     The file name without extension of a file in robi format
	 *                     that specifies the Robot's image.
	 */
	 ...
	 *
	 * @param robotImage
	 *                       a BufferedImage containing the robot image. It does not
	 *                       need to be to scale since it will be scaled to the
	 *                       appropriate size.
	 */
	...
	/**
	 * @param fileName
	 *                     the name of the file containing the Robot image, without
	 *                     the ".robi" extension.
	 * @param xPos
	...
	/**
	 * Sets the window's background color given the red, green and blue components
	 * of the new color. The components are specified as an integer between 0 and
	 * 255.
	 *
	 * @param r
	...
	private void segmentTo(Segment segment, boolean forwards) {
		...
		if (!Double.isNaN(startAngle))
			turn(getAngleToTurn(startAngle + directionAdjustment));
		...
		try {
			while (t < 1.0F) {
				...
				if (!Double.isNaN(endAngle))
					setAngle(Math.toDegrees(endAngle + directionAdjustment));
	...
	public void drawRainbowLine(int distance) {
		int traveled = 0;
		int red = 0;
		int green = 0;
		int blue = 0;

		repeat: while (traveled < distance) {
			for (int i = 0; i <= 51; i++) {
				setPenColor(255, 0, blue += 5);
				move(1);
